    ...
    switch (active) {
        ...
        return {
          ...
          visual: (
            <div className="h-[300px] md:h-[340px] rounded-2xl border border-gray-100 bg-gradient-to-br from-slate-50 to-white overflow-hidden p-4">
              <div className="px-2.5 py-2 flex items-center gap-2.5 border-b border-gray-100"><Image src="/logo.png" alt="Dataxx" width={28} height={28} className="h-7 w-7 rounded-md ring-1 ring-gray-200" /><div className="min-w-0"><div className="text-secondary font-semibold leading-tight text-[13px]">Dataxx</div><div className="text-[10px] text-gray-500">Fiche identité</div></div></div>
      ...
      case "contacts":
        return {
          ...
          visual: (
            <div className="flex flex-col items-center justify-center gap-4 px-4 sm:px-6">{[{img:avatarDG,role:"Directeur général"},{img:avatarDM,role:"Directeur Marketing"},{img:avatarRS,role:"Responsable Sponsoring"}].map((p)=> (<div key={p.role} className="bg-primary/5 rounded-2xl w-full max-w-[520px] min-h-14 flex items-center gap-3 border border-primary/10 px-4 py-2"><Image src={p.img} alt="Avatar IA" width={32} height={32} className="w-8 h-8 rounded-full object-cover border border-primary/20" /><div className="flex-1 text-left"><div className="text-secondary text-sm font-medium">{p.role}</div><div className="flex flex-wrap items-center gap-2 pt-0.5"><button className="text-[11px] px-2.5 py-1 rounded-full border border-primary/30 text-primary bg-white">Révéler le téléphone</button><button className="text-[11px] px-2.5 py-1 rounded-full meetsponsors-gradient text-white">Révéler l'email</button></div></div></div>))}</div>
          ),
        };
      case "emailing":
        return {
